...
describe('GetAuhenticationUseCase', () => {
  it('should orchestrating the get authentication action correctly', async () => {
    ...
    const expectedAuthentication = new Auth({
    ...
    expect(mockUserRepository.getPasswordByUsername).toBeCalledWith('user');
    expect(mockPasswordHash.comparePassword).toBeCalledWith('secret_password', 'encrypted_password');
    expect(mockUserRepository.getIdByUsername).toBeCalledWith('user');
    expect(mockAuthenticationTokenManager.createAccessToken)
      .toBeCalledWith({ username: 'user', id: 'user-123'});
